...
export default async function SapChieuPage({ searchParams }: PageProps) {
  ...
  const [categories, movies] = await Promise.all([
    ...
    getNewUpdatedMovies({ page, limit: 24, type_list: "sap-chieu", sort_type: sortType }).catch(() => ({ items: [] })),
  ]);
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      ...
      <main className="mx-auto max-w-7xl px-4 py-6">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">Sắp Chiếu</h1>
            <p className="text-sm text-zinc-400">Những bộ phim sắp ra mắt</p>
          </div>
        </div>
